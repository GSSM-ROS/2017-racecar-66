...
class Pot_Field():				 #We are in RCS class. Everything we are doing is in this class. There are three gssm students in RCS and three methods in this class. A triangle has three sides. Coincidence? I think not. Illuminati confirmed.
    ...
    def drive(self, data):			 #Receives scans and controls steering and speed
        ...
        '''
        self.send=Point()	 #declares self.send as an ackermanndrivestamped. allows us to decide speed and turning angle
        ...
        self.send.y=self.drive_constant * (self.back_force-pushback) 			#sets net force equal to backforce - pushback
        if self.send.y > self.max_speed:
            self.send.y = self.max_speed
        elif self.send.y < -self.max_speed:
            self.send.y = -self.max_speed
        self.send.x = self.turn_constant * turn 				#sets net force = turn* a constant that scales the turning angle according to distance.
        if self.send.x < 0:								#Checks if the speed is negative
            self.send.x = -self.send.x			#reverses steering angle to turn around while in reverse
    ...
    def __init__(self):				 #Defines the initializer
        ...
        self.pub = rospy.Publisher("/wall_sum", Point, queue_size=6) 	#creates a publisher to the location that controls speed and steering angle.
        self.safe_pub = rospy.Publisher("/safe", Bool, queue_size=1)
        rospy.on_shutdown(self.die)								#calls back die() on death.

        self.back_force = 0		 	 #constant force from the back that is the speed when there is no pushback assuming its value is less than the value for the max speed of the robot
        self.point_charge = rospy.get_param("wall_point")#sets a q value for objects. This is adjusted to prevent few points from stopping the robot completely.
        self.turn_constant = 1			 #Turn constant can be modified to scale rate of turning up or down.
        self.drive_constant = 1			 #drive constant can be modified to change effect of pushback. Scales up or down.
        self.max_speed = 2			 #maximum speed the robot will go
